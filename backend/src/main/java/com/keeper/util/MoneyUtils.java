...
/**
 * Utility for handling financial amounts with Banker's Rounding (HALF_EVEN). All currency in the DB
 * is stored as minor units (e.g., cents) using BIGINT.
 */
public class MoneyUtils {

  /**
   * Converts a major unit BigDecimal (e.g., 10.50 USD) to minor units (1050) using Banker's
   * Rounding (HALF_EVEN).
   *
  ...
  public static long toMinorUnits(BigDecimal amount, int minorUnits) {
    ...
    return amount.multiply(multiplier).setScale(0, RoundingMode.HALF_EVEN).longValue();
  }
